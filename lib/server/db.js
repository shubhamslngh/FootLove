...
const SCHEMA_STATEMENTS = [
  `CREATE TABLE IF NOT EXISTS users (
    ...
    profile_image_data_url LONGTEXT NULL,
  ...
  `CREATE TABLE IF NOT EXISTS bookings (
    ...
    guest_username VARCHAR(80) NULL,
  ...
  return normalizeData({
    users: userRows.map((row) =>
      compact({
        ...
        profileImageDataUrl: row.profile_image_data_url || undefined,
    ...
    bookings: bookingRows.map((row) =>
      compact({
        ...
        guestUsername: row.guest_username || undefined,
...
async function writeNormalizedDb(connection, sourceData) {
  ...
  await upsert(
    ...
    "bookings",
    [
      "id", "match_id", "user_id", "guest_name", "guest_phone",
      "guest_username", "slot_role", "status", "payment_status",
      "payment_reference", "team",
      "confirmed_by_user_id", "confirmed_at", "rejected_by_user_id",
      "rejected_at", "created_at",
    ],
    data.bookings.map((booking) => [
      booking.id, booking.matchId, booking.userId || null,
      booking.guestName || null,
      booking.guestPhone || null, booking.guestUsername || null,
  ...
  await upsert(
    ...
    [
      ...
      "booking_id", "meta", "is_read", "read_at", "created_at",
    ],
    data.notifications.map((notification) => [
      notification.id, notification.userId, notification.type,
      notification.title, notification.message,
      notification.matchId || null, notification.bookingId || null,
      notification.meta ? JSON.stringify(notification.meta) : null,
...
async function ensureMysqlDatabase() {
  ...
  await ensureColumn(connection, "notifications", "meta", "JSON NULL");
  await ensureColumn(
        connection,
        "users",
        "profile_image_data_url",
      ...
      await ensureColumn(
        ...
        "bookings",
        "guest_name",
        "VARCHAR(160) NULL AFTER user_id",
      );
      await ensureColumn(
        connection,
        "bookings",
        "guest_phone",
        "VARCHAR(20) NULL AFTER guest_name",
      );
      await ensureColumn(
        connection,
        "bookings",
        "guest_username",
        "VARCHAR(80) NULL AFTER guest_phone",
      );
      await connection.execute(
        "ALTER TABLE bookings MODIFY user_id VARCHAR(64) NULL",
      );
